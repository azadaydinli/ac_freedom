...
Provides toggle switches for AC features:
  - Display: visible in HA UI (screen on/off)
  - Sleep, Health, Clean, Eco: hidden in HA UI, only visible
    when bridged to HomeKit (appear in the climate card)
"""
...
# key -> (name, icon, local_attr, cloud_key, hidden)
SWITCH_TYPES = {
    "display": ("Display", "mdi:monitor", "display", AC_SCREEN_DISPLAY, False),
    "sleep": ("Sleep Mode", "mdi:power-sleep", "sleep", AC_SLEEP, True),
    "health": ("Health", "mdi:air-filter", "health", AC_HEALTH, True),
    "clean": ("Self Clean", "mdi:vacuum", "clean", AC_CLEAN, True),
    "mildew": ("Eco", "mdi:water-off", "mildew", AC_MILDEW_PROOF, True),
}
...
async def async_setup_entry(
    ...
    if conn_mode == CONN_CLOUD:
        ...
        for dev in data.get("devices", []):
            ...
            for key, (name, icon, _, cloud_key, hidden) in SWITCH_TYPES.items():
                entities.append(
                    CloudSwitch(coordinator, dev, info, key, name, icon, cloud_key, hidden)
                )
    else:
        for dev_entry in data.get("local_devices", []):
            info = _local_device_info(dev_entry["info"])
            raw = dev_entry["info"]
            for key, (name, icon, state_attr, _, hidden) in SWITCH_TYPES.items():
                entities.append(
                    LocalSwitch(
                        dev_entry["coordinator"], info, raw,
                        key, name, icon, state_attr, hidden,
                    )
                )
...
class LocalSwitch(CoordinatorEntity, SwitchEntity):
    ...
    def __init__(
        ...
        key: str, name: str, icon: str, state_attr: str, hidden: bool,
    ) -> None:
...
class CloudSwitch(CoordinatorEntity, SwitchEntity):
    ...
    def __init__(
        ...
        key: str, name: str, icon: str, cloud_key: str, hidden: bool,
    ) -> None:
        super().__init__(coordinator)
        self._device = device
